...
const EmailLinking: React.FC = () => {
  ...
  const handleProviderSelect = (p: { id: string }) => {
  ...
  const handleSyncClick = async () => {
    ...
    setLoading(true);
    try {
      if (selectedProvider === 'google' || selectedProvider === 'outlook') {
        const response = await api.get(`/emails/link/${selectedProvider}`, {
          params: { email },
        });
        ...
        <div className="max-w-xl w-full space-y-12 animate-fade-in">
          <div className="text-center space-y-4">
            ...
            <p className="text-gray-500 font-medium">
              {success
                ? 'O Agente começará a sincronizar a sua inbox agora.'
                : 'Ligue o seu email para que o Agente IA possa recolher faturas automaticamente.'}
            </p>
          </div>
                    ...
                    >
                      <div className="flex items-center space-x-6">
                        <div className={`w-14 h-14 rounded-2xl ${p.color} flex items-center justify-center border border-inherit`}>
                          {p.icon ? <img src={p.icon} alt={p.name} className="w-7 h-7" /> : <Globe size={24} className="text-gray-400" />}
                        </div>
                        <span className="font-black text-gray-900 group-hover:text-blue-600 transition-colors uppercase text-xs tracking-wider">
                          {p.name}
                        </span>
                      </div>
                  ...
                  <div className="flex items-start space-x-4 p-4 rounded-2xl bg-blue-50/50 border border-blue-100">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-[11px] text-gray-500 font-medium leading-relaxed">
                        Gmail e Outlook usam OAuth2. IMAP genérico requer credencial ou app password do provedor.
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="flex items-center justify-center space-x-10 text-[11px] font-extrabold text-gray-400 uppercase tracking-widest opacity-60">
            <div className="flex items-center space-x-2">
              <Lock size={14} /> <span>Encriptação SSL</span>
            </div>
            <div className="flex items-center space-x-2">
              <CheckCircle2 size={14} /> <span>GDPR Compliant</span>
            </div>
            <div className="flex items-center space-x-2">
              <MessageSquare size={14} /> <span>Suporte 24/7</span>
            </div>
          </div>
        </div>
      </div>
