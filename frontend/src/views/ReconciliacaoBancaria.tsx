...
const ReconciliacaoBancaria: React.FC = () => {
  const bankTransactions = [
    { id: '1', date: '12 Out', name: 'PAG. EDP COMERCIAL SA', amount: '-€145,20', isMatched: true },
    { id: '2', date: '11 Out', name: 'WORTEN EQUIPAMENTOS', amount: '-€45,99', isMatched: true },
    { id: '3', date: '10 Out', name: 'ÁGUAS DO PORTO EM', amount: '-€82,15', isMatched: false },
    { id: '4', date: '08 Out', name: 'UBER RIDE IRELAND', amount: '-€12,50', isMatched: false },
  ];

  const aiSuggestions = [
    { id: '101', vendor: 'EDP Comercial', doc: 'FATURA_EDP_JANEIRO.PDF', confidence: 0.98, matchedTo: '1' },
    { id: '102', vendor: 'Worten PT', doc: 'RECIBO_WORTEN_01.PDF', confidence: 0.95, matchedTo: '2' },
  ];
        ...
        <div className="flex-1 p-10 space-y-10 border-r border-gray-200 overflow-y-auto no-scrollbar">
           ...
           <div className="space-y-4">
              {bankTransactions.map((tx) => (
                <div key={tx.id} className={`flex items-center p-6 rounded-[32px] border border-gray-100 bg-white hover:bg-gray-50 transition-all group shadow-sm ${tx.isMatched ? 'opacity-50 grayscale-0' : 'border-blue-500/10'}`}>
                   ...
                   <div className="flex-1 ml-6">
                      <h4 className="font-bold text-[14px] text-gray-900">{tx.name}</h4>
                      <p className="text-[11px] text-gray-500 font-bold tracking-widest uppercase">{tx.date}</p>
                   </div>
                   <div className="text-right">
                      <p className="font-black text-[16px] text-gray-900">{tx.amount}</p>
                      {tx.isMatched && (
                  ...
                  <div key={sug.id} className="relative group">
                     ...
                     <div className="p-8 rounded-[40px] bg-gray-50/50 border border-gray-100 hover:border-blue-500/30 transition-all space-y-6">
                        <div className="flex justify-between items-start">
                           <div className="space-y-1">
                              <span className="text-[9px] font-black uppercase tracking-widest px-2 py-0.5 bg-blue-500 text-white rounded-md">Confiança {sug.confidence * 100}%</span>
                              <h4 className="font-black text-lg pt-1 text-gray-900">{sug.vendor}</h4>
                           </div>
                        ...
                        <div className="bg-white rounded-2xl p-4 flex items-center space-x-3 border border-gray-100 border-dashed">
                           ...
                           <div className="flex-1">
                              <p className="text-xs font-bold text-gray-700 truncate">{sug.doc}</p>
                              <p className="text-[10px] text-gray-400 font-extrabold uppercase">Fatura Associada</p>
                           </div>
                        </div>

                        <div className="flex space-x-3">
                           <button className="flex-1 py-3.5 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl text-[12px] font-black shadow-xl shadow-blue-500/20 active:scale-95 transition-all">Aprovar Sugestão</button>
