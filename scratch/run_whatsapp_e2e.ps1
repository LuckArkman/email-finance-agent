...
Write-Host "=== TESTE: WhatsApp Text Routing (Instrução para Agente LLM) ==="
try {
    $payload = @{
        sender_phone = "351912345678"
        text = "Quantas faturas atrasadas tenho?"
    }
    $jsonPayload = $payload | ConvertTo-Json
    $response = Invoke-RestMethod -Uri "http://localhost:5000/api/hermes/whatsapp/message" -Method Post -Body $jsonPayload -ContentType "application/json"
    Write-Host "Resposta do Servidor: " ($response | ConvertTo-Json)
} catch {
    Write-Host "WhatsApp Message Error: $_"
}

Write-Host "`n=== TESTE: WhatsApp Image Routing (Pipeline OCR) ==="
try {
